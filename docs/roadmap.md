...
| **M8** (in progress) | ✅ RTXDI (full ReSTIR DI, verified), ✅ FBX/Assimp import (Arcade, verified), ✅ SDF grids NDSDF+SBS (verified); ✅ NRD (NRD 3.1.0 library as wasm + its HLSL via Slang; ReLAX/ReBLUR/motion vectors), ✅ WARDiffPathTracer (forward and backward graphs verified vs native; backward kernels on a pinned pre-refactor Slang), ✅ SDFEditor (GUI layer verified vs native; picking, primitive edits, undo/redo); ✅ SDF grids (all 4 types verified), ✅ Mogwai functional viewer (render loop + present, smoke-tested); ⏳ ImGui-wasm node editor + WebGL2 raster (stretch tooling) | pass-rate report + parity matrix finalized (§7.2); every rendering-feature item verified-vs-native or with a documented blocker |
