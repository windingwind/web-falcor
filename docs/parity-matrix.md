...
| Gui (Dear ImGui) | 🔶 | DOM `UIWidgets` (text/button/checkbox/slider/dropdown/group); `renderUI` ported on 26 of 28 pass classes (native controls via text/button/checkbox/slider/dropdown/group; "Output size"/"Size in pixels" wired through `RenderPass.requestRecompile()`; InvalidPixelDetection and RenderPassTemplate have no controls natively); TextRenderer/Font ✅ (`Utils/UI/`: upstream `dejavu-sans-mono-14.bin/.dds` atlas fetched from the served Falcor tree, upstream `TextRenderer.3d.slang` via a struct-varyings override, SrcAlpha blending — §9.16); PixelZoom ⏳ |
...
15. **Raster G-/V-buffer "extra" channels are render targets, not ROV stores.** WebGPU has
   ...
   as SV_TARGET1/2. The GBufferRaster `vbuffer` channel needs SV_PrimitiveID and is not
   offered. Also: upstream declares raster varyings HLSL `linear` (perspective-correct),
