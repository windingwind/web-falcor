...
| Gui (Dear ImGui) | 🔶 | DOM `UIWidgets` (text/button/checkbox/slider/dropdown/group); `renderUI` ported on 25 of 28 pass classes (native controls via text/button/checkbox/slider/dropdown/group; "Output size"/"Size in pixels" wired through `RenderPass.requestRecompile()`; RTXDI-options controls ⏳); TextRenderer/Font/PixelZoom ⏳ |
