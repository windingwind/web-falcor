...
| SDFEditor | ⏳ + 🟠 oracle | not built: `.sdf`/`.sdfg` IO now exists (see the SDF grids row), so what remains is runtime SDF editing/bake (§8.4) + the interactive UI. Upstream flagship scene asset absent from the media drop, so native oracle impossible anyway |
...
| Image IO (Bitmap/EXR read+write, image save) | 🟡 | `.hdr`/DDS-BC/`.exr` decode (EXR via parse-exr, wired into ImageLoader + EnvMap + ErrorMeasure); DDS also parses plain 24/32-bit RGB(A) surfaces (channel-mask swizzle to RGBA8, e.g. the font atlas); `.tga` decode ✅ (`Utils/Image/TGADecoder.ts`: uncompressed and RLE true-colour, grayscale and colour-mapped, both origins — the web substitute for FreeImage, used by pbrt and FBX/OBJ textures); EXR write (uncompressed float scanlines, bit-exact round-trip) feeds the viewer capture; unified Bitmap class ⏳ |
