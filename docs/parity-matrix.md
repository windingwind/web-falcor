...
| Importer: glTF | ✅ | TS importer: meshes, skinning, morph targets, animations, cameras, lights; `KHR_mesh_quantization` ✅ (signed and normalized byte/short attributes, integer positions placed by the node transform) together with `KHR_texture_transform` (baked into the primitive's uvs — each primitive has one material, so this is exact); verified against the same model's uncompressed variant: 90% of the shaded surface agrees to 7e-5 in position, 0.2° in normal and 9e-5 in uv. KTX2 and Draco ⏳ (both need third-party transcoders) |
