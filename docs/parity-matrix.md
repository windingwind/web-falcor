...
| StandardMaterial | ✅ | verified across the oracle suite. Open finding 🟡: the specular+normal-mapped tutorial.pyscene materials show a mode-independent ~6% PT radiance deficit vs native (found by the texLOD cross-oracle; suspected normal/specular texture-slot semantics on the pyscene loadTexture path — under investigation) |
