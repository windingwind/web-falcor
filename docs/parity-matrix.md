...
| LightProfile (IES) | ⏳ | dummy binding only; no IES loader/bake. Low priority: in this Falcor drop a material only gets a light profile through the USD importer (`inputs:shaping:ies:file`) — no python setter — and the web USD path (tinyusdz) exposes no lights, so nothing could reach it |
