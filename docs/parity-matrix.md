...
| MERL / MERLMix / RGL measured materials | 🟡 MERL | `Scene/Material/MERLFile.ts` reads the `.binary` database layout (90x90x180 half/difference bins, per-channel scales, inf/NaN sanitising) plus the optional `.json` diffuse/specular fit, and `MERLMaterial` renders through the upstream `MERLCommon::eval` and `DiffuseSpecularBRDF` sampling. §9: the samples and the precomputed albedo LUT share the one material buffer (no binding arrays in WGSL; the LUT is float data the packed RGBA8 texture array cannot hold), so `bufferID`/`texAlbedoLUT` carry byte offsets; the LUT is integrated with the same `BSDFIntegrator` native uses. Verified against analytic BRDFs written in the MERL format by `tools/gen-assets.mjs` (the real database is licence-gated): constant-albedo evaluation to 1.4e-8, integrated albedo exactly 0.6, and a per-bin index probe hitting every bin the CPU mapping predicts. MERLMix (per-texel BRDF index) and RGL ⏳ |
