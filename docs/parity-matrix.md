...
| Curve geometry (LSS) / hair | ✅ core | USD BasisCurves import (USDA text) → CurveTessellation port → linear-swept-sphere segments intersected in the software query (upstream CurveIntersector/Han19, flat segment loop — segment-BVH is the perf follow-up). two_curves.pyscene FaceNormals match native pixel-exactly (0/65536); shaded curves (default HairMaterial, Chiang16) match native through MPT and the full PathTracer (USE_CURVES) at 64spp (bias ≤1.1e-3). `.hair` files + segment BVH for hair-scale counts ⏳ |
