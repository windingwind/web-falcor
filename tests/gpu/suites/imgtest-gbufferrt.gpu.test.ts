/**
 * OVERALL VERIFY — upstream image test: the UNMODIFIED GBufferRTInline.py
...
gpuTest("ImageTestGBufferRT.matchesNativeOracle", async ({ device }) => {
    await initScripting("/node_modules/pyodide");
    const source = await (await fetch("/Falcor/tests/image_tests/renderpasses/graphs/GBufferRTInline.py")).text();
    ...
    for (const [channel, components, tol, badTol, isFp16] of kCompare) {
        ...
        const res = await fetch(`/tests/oracle/out-native/oracle-imgtest-gbufferrt.GBufferRT.${channel}.0.exr`);
        ...
        console.error(`# imgtestGBufferRT.${channel}: mean=${mean.toExponential(2)} bad=${badPixels}`);
