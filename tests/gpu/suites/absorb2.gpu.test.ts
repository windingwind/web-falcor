...
gpuTest("Absorb2.matchesNativeOracle", async ({ device }) => {
    const size = 128;
    await initScripting("/node_modules/pyodide");
    const sceneSource = await (await fetch("/tests/oracle/assets/oracle-absorb2.pyscene")).text();
    ...
    const res = await fetch("/tests/oracle/out-native/oracle-absorb2.PathTracer.color.0.exr");
    ...
    console.error(`# absorb2 center web=${webPx(64, 64).map((v) => v.toFixed(5))} nat=${natPx(64, 64).map((v) => v.toFixed(5))}`);
    console.error(`# absorb2 bg web=${webPx(5, 5).map((v) => v.toFixed(5))} nat=${natPx(5, 5).map((v) => v.toFixed(5))}`);
    ...
    console.error(`# absorb2: mean=${mean.toExponential(2)} bad=${bad}`);
