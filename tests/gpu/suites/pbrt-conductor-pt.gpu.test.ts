/**
 * M7 material zoo: PBRTConductorMaterial through the static-dispatch factory
 * (WEBFALCOR_MTL_PBRT_CONDUCTOR) — path-traced PBRT-conductor quad vs the native hardware-DXR
 * oracle. The scene mirrors tests/oracle/assets/oracle-pt-pbrtconductor.pyscene:
 * TriangleMesh.createQuad(1,1) rotated 90° about X into [0,1]^2 at z=0.
 *
 * Regenerate the oracle with:
 *   Falcor/build/linux-gcc/bin/Debug/Mogwai --script tests/oracle/render-native-pt-pbrtconductor.py --headless
 */
...
gpuTest("PBRTConductorPathTracer.matchesNativeOracle", async ({ device }) => {
    const size = 256;
    ...
    const scene = new Scene(
        ...
        [
            {
                header: { materialType: MaterialType.PBRTConductor },
                basic: {
                    baseColor: new float4(0.2, 0.9, 1.4, 1), // eta
                    specular: new float4(0.3, 0.5, 0, 0), // anisotropic roughness .rg
                    transmission: new float3(3.9, 2.5, 2.1), // k
                },
            },
        ],
    ...
    const graph = new RenderGraph(device, "PBRTConductorPTGraph");
    ...
    const res = await fetch("/tests/oracle/out-native/oracle-pt-pbrtconductor.MinimalPathTracer.color.0.exr");
    ...
    console.error(`# pbrtConductorPT: meanAbs=${mean.toExponential(2)} rel=${rel.toExponential(2)} bad=${badPixels}`);
