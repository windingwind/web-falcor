/**
 * OVERALL VERIFY — upstream image test: the UNMODIFIED VBufferRT.py graph over
 * the unmodified cornell_box.pyscene; depth/mvec/viewW/mask channels compared
 * against native EXR captures per-pixel.
 *
 * Regenerate the oracle with:
 *   Falcor/build/linux-gcc/bin/Debug/Mogwai --script tests/oracle/render-native-imgtest-vbuffer.py --headless
 */
...
gpuTest("ImageTestVBuffer.matchesNativeOracle", async ({ device }) => {
    const size = 256;
    ...
    for (const [channel, components, tol, badTol] of [
        ...
        const res = await fetch(`/tests/oracle/out-native/oracle-imgtest-vbuffer.VBufferRT.${channel}.0.exr`);
        ...
        console.error(`# imgtestVBuffer.${channel}: mean=${mean.toExponential(2)} bad=${badPixels}`);
