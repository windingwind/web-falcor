/**
 * OVERALL VERIFY — upstream image test: the UNMODIFIED WhittedRayTracer.py
 * graph (GBufferRT -> Whitted megakernel: perfect reflect/refract chains,
 * RayCones Unified texLOD, per-light shadow rays -> ToneMapper) over
 * cornell_box.pyscene, diffed against the native PNG capture.
 *
 * Regenerate: Mogwai --script tests/oracle/render-native-imgtest-whitted.py --headless
 */
...
gpuTest("ImageTestWhitted.matchesNativeOracle", async ({ device }) => {
    await initScripting("/node_modules/pyodide");
    const source = await (await fetch("/Falcor/tests/image_tests/renderpasses/graphs/WhittedRayTracer.py")).text();
    ...
    const blob = await (await fetch("/tests/oracle/out-native/oracle-imgtest-whitted.ToneMapper.dst.0.png")).blob();
    ...
    mse /= size * size * 3;
    console.error(`# imgtestWhitted: mse=${mse.toExponential(2)} bad=${bad} bands=${bandMse.map((v) => (v / (size * 64 * 3)).toExponential(1)).join(",")}`);
