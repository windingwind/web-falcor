/**
 * OVERALL VERIFY: the UNMODIFIED upstream convergence_test.pyscene — colored
 * emissive area lights, metals/mirrors/glass (specular transmission, IoR),
 * DistantLight, spheres+quads — through the Pyodide bridge + PathTracer
 * (LightBVH NEE).
 *
 * Regenerate the oracle with:
 *   Falcor/build/linux-gcc/bin/Debug/Mogwai --script tests/oracle/render-native-convergence.py --headless
 */
...
gpuTest("ConvergenceTest.matchesNativeOracle", async ({ device }) => {
    const size = 256;
    await initScripting("/node_modules/pyodide");
    const source = await (await fetch("/Falcor/media/test_scenes/convergence_test.pyscene")).text();
    ...
    const graph = new RenderGraph(device, "ConvergenceGraph");
    ...
    const res = await fetch("/tests/oracle/out-native/oracle-convergence.PathTracer.color.0.exr");
