/**
 * M6 EXIT TEST — MinimalPathTracer vs native oracle.
 * Native: hardware DXR RT pipeline (VBufferRT -> MinimalPathTracer, 1 frame).
 * Web: same graph over software BVH ray queries + megakernel override.
 * Same scene (quad.gltf + point light), camera and RNG (TinyUniform,
 * frameCount 0) => radiance matches to floating-point tolerance.
 *
 * Regenerate the oracle with:
 *   Falcor/build/linux-gcc/bin/Debug/Mogwai --script tests/oracle/render-native-pt.py --headless
 */
...
gpuTest("MinimalPathTracer.matchesNativeOracle", async ({ device }) => {
    const size = 256;
    ...
    const graph = new RenderGraph(device, "PTGraph");
    ...
    const res = await fetch("/tests/oracle/out-native/oracle-pt.MinimalPathTracer.color.0.exr");
    ...
    const relMean = sum / Math.max(refSum, 1e-6);
    console.error(`# minimalPT: meanAbs=${mean.toExponential(2)} rel=${relMean.toExponential(2)} bad=${badPixels}`);
