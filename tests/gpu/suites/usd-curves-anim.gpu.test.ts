/**
 * USD curve vertex caches like native's CachedCurve + AnimatedVertexCache: the
 * time-sampled points (samples before time code 1 dropped, as natively) tessellate
 * per sample, positions lerp (widths stay), the curves loop over their own length and
 * their BVH is rebuilt. Compared with native GBufferRT captures at 0.25s, 1.0s and
 * 2.0s. Both curves are cached: native's update writes cached curves from the start
 * of the scene's curve buffer, so mixing static and cached curves breaks natively.
 *
 * Regenerate the oracle with:
 *   Falcor/build/linux-gcc/bin/Release/Mogwai --script tests/oracle/render-native-usd-curves-anim.py --headless
 */
...
gpuTest("UsdCurvesAnim.matchesNativeOracle", async ({ device }) => {
    const [w, h] = [256, 128];
    ...
    const native = async (c: string, frame: number) => (parseExr(await (await fetch(`/tests/oracle/out-native/usd-curves-anim.GBufferRT.${c}.${frame}.exr`)).arrayBuffer(), 1015) as { data: Float32Array }).data;
    ...
    for (const frame of [6, 24, 48]) {
        ...
        console.error(`# usdCurvesAnim frame ${frame}: hits ${hits}, mask mismatches ${maskMismatch}, ${errors.join(", ")}`);
