...
gpuTest("RenderUI.everyPassExposesFlippableControls", async ({ device }) => {
    ...
    // Native passes without any renderUI: InvalidPixelDetection, RenderPassTemplate; BSDFViewer and
    // TestRtProgram only list controls once a scene is set (BSDFViewer covered below).
    expectEq(without.every((t) => ["InvalidPixelDetectionPass", "RenderPassTemplate", "BSDFViewer", "TestRtProgram"].includes(t)), true, `unexpected passes without controls: ${without.join(", ")}`);
