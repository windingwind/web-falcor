/**
 * An animated USD PointInstancer: time-sampled positions/orientations give each
 * instance its own node animation (native's createPointInstanceKeyframes: the instance
 * matrices at the positions' sample times, decomposed), with the prototype subtree as a
 * node chain below it. Compared with native GBufferRT captures at 0.5s, 1.25s and 2.5s.
 *
 * Regenerate the oracle with:
 *   Falcor/build/linux-gcc/bin/Release/Mogwai --script tests/oracle/render-native-usd-instancer-anim.py --headless
 */
...
gpuTest("UsdInstancerAnim.matchesNativeOracle", async ({ device }) => {
    const [w, h] = [192, 96];
    await initScripting("/node_modules/pyodide");
    const source = await (await fetch("/tests/oracle/assets/usd-instancer-anim.pyscene")).text();
    ...
    const graph = new RenderGraph(device, "UsdInstancerAnim");
    ...
    const native = async (c: string, frame: number) => (parseExr(await (await fetch(`/tests/oracle/out-native/usd-instancer-anim.GBufferRT.${c}.${frame}.exr`)).arrayBuffer(), 1015) as { data: Float32Array }).data;
    ...
    for (const frame of [12, 30, 60]) {
        ...
        console.error(`# usdInstancerAnim frame ${frame}: hits ${hits}, mask mismatches ${maskMismatch}, ${errors.join(", ")}`);
