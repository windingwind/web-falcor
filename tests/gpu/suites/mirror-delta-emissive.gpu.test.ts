/**
 * M7: delta reflection under emissive lighting (LightBVH NEE + mirror) vs
 * the native PathTracer.
 * Regenerate: Mogwai --script tests/oracle/render-native-mirror-em.py --headless
 */
...
gpuTest("MirrorDeltaEmissive.matchesNativeOracle", async ({ device }) => {
    const size = 256;
    await initScripting("/node_modules/pyodide");
    const source = await (await fetch("/tests/oracle/assets/oracle-mirror-em.pyscene")).text();
    ...
    const graph = new RenderGraph(device, "IsoMirrorEmGraph");
    ...
    const res = await fetch("/tests/oracle/out-native/oracle-mirror-em.PathTracer.color.0.exr");
    ...
    console.error(`# mirrorEmPT: meanAbs=${(sum / (size * size * 3)).toExponential(2)} bad=${badPixels}`);
