/**
 * M7: emissive surfaces as lights — path-traced two-quad scene (blue quad +
 * emissive quad in front) vs the native hardware-DXR oracle. Exercises
 * multi-mesh BVH, per-mesh materials and USE_EMISSIVE_LIGHTS.
 *
 * Regenerate the oracle with:
 *   Falcor/build/linux-gcc/bin/Debug/Mogwai --script tests/oracle/render-native-pt-emissive.py --headless
 */
...
gpuTest("EmissivePathTracer.matchesNativeOracle", async ({ device }) => {
    const size = 256;
    ...
    const graph = new RenderGraph(device, "EmissivePTGraph");
    ...
    const res = await fetch("/tests/oracle/out-native/oracle-pt-emissive.MinimalPathTracer.color.0.exr");
    ...
    console.error(`# emissivePT: meanAbs=${mean.toExponential(2)} rel=${rel.toExponential(2)} bad=${badPixels}`);
