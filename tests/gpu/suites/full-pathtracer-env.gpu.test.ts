/**
 * M7: full PathTracer with environment light — EnvMapSampler hierarchical
 * importance sampling (upstream setup kernel + generateMips) vs the native
 * hardware-DXR PathTracer.
 *
 * Regenerate the oracle with:
 *   Falcor/build/linux-gcc/bin/Debug/Mogwai --script tests/oracle/render-native-fullpt-env.py --headless
 */
...
gpuTest("FullPathTracerEnv.matchesNativeOracle", async ({ device }) => {
    const size = 256;
    ...
    const res = await fetch("/tests/oracle/out-native/oracle-fullpt-env.PathTracer.color.0.exr");
    ...
    console.error(`# fullPTEnv: meanAbs=${mean.toExponential(2)} rel=${rel.toExponential(2)} bad=${badPixels}`);
