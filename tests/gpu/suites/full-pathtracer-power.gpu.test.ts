 ...
 * MIS, EmissivePowerSampler alias table, bit-replicated incl. the mt19937
 * shuffle) vs the native hardware-DXR PathTracer on the emissive two-quad scene.
 *
 * Regenerate the oracle with:
 *   Falcor/build/linux-gcc/bin/Debug/Mogwai --script tests/oracle/render-native-fullpt-power.py --headless
 */
...
gpuTest("FullPathTracerPower.matchesNativeOracle", async ({ device }) => {
    const size = 256;
    ...
    const graph = new RenderGraph(device, "FullPTPowerGraph");
    graph.onResize(size, size);
    graph.addPass(createPass(device, "VBufferRT", { useAlphaTest: false }), "VBufferRT");
    graph.addPass(createPass(device, "PathTracer", { samplesPerPixel: 1, emissiveSampler: "Power" }), "PathTracer");
    ...
    const res = await fetch("/tests/oracle/out-native/oracle-fullpt-power.PathTracer.color.0.exr");
    ...
    console.error(`# fullPTPower: meanAbs=${mean.toExponential(2)} rel=${rel.toExponential(2)} bad=${badPixels}`);
