/**
 * M7: environment map as light + background — path-traced quad with a
 * gradient lat-long env map vs the native hardware-DXR oracle.
 *
 * Regenerate the oracle with:
 *   Falcor/build/linux-gcc/bin/Debug/Mogwai --script tests/oracle/render-native-pt-env.py --headless
 */

import { EnvMap, GltfImporter, LightType, RenderGraph, createPass, float3 } from "@web-falcor/falcor";
import "@web-falcor/render-passes";
import parseExr from "parse-exr";
import { gpuTest, expectEq } from "../harness/registry.js";

gpuTest("EnvMapPathTracer.matchesNativeOracle", async ({ device }) => {
    const size = 256;
    ...
    scene.setEnvMap(await EnvMap.createFromUrl(device, "/tests/oracle/assets/gradient.hdr"));
    ...
    const graph = new RenderGraph(device, "EnvPTGraph");
    ...
    const res = await fetch("/tests/oracle/out-native/oracle-pt-env.MinimalPathTracer.color.0.exr");
    ...
    console.error(`# envmapPT: meanAbs=${mean.toExponential(2)} rel=${rel.toExponential(2)} bad=${badPixels}`);
