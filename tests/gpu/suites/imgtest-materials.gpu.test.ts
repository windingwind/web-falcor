/**
 * OVERALL VERIFY — upstream image test: the UNMODIFIED PathTracerMaterials.py
...
gpuTest("ImageTestMaterials.matchesNativeOracle", async ({ device }) => {
    await initScripting("/node_modules/pyodide");
    const source = await (await fetch("/Falcor/tests/image_tests/renderpasses/graphs/PathTracerMaterials.py")).text();
    const [graph] = await runGraphScript(device, source);

    const sceneSource = await (await fetch("/Falcor/media/test_scenes/material_test.pyscene")).text();
    ...
    const blob = await (await fetch("/tests/oracle/out-native/oracle-imgtest-materials.ToneMapper.dst.0.png")).blob();
    ...
    mse /= size * size * 3;
    console.error(`# imgtestMaterials: mse=${mse.toExponential(2)} bad=${bad} bands=${bandMse.map((v) => (v / (size * 64 * 3)).toExponential(1)).join(",")}`);
