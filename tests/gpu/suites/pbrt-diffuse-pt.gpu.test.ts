/**
 * M7 material zoo: PBRTDiffuseMaterial through the static-dispatch factory
 * (WEBFALCOR_MTL_PBRT_DIFFUSE) — path-traced PBRT-diffuse quad vs the native hardware-DXR
 * oracle. The scene mirrors tests/oracle/assets/oracle-pt-pbrtdiffuse.pyscene:
 * TriangleMesh.createQuad(1,1) rotated 90° about X into [0,1]^2 at z=0.
 *
 * Regenerate the oracle with:
 *   Falcor/build/linux-gcc/bin/Debug/Mogwai --script tests/oracle/render-native-pt-pbrtdiffuse.py --headless
 */
...
gpuTest("PBRTDiffusePathTracer.matchesNativeOracle", async ({ device }) => {
    const size = 256;
    ...
    const scene = new Scene(
        ...
        [
            {
                header: { materialType: MaterialType.PBRTDiffuse },
                basic: {
                    baseColor: new float4(0.5, 0.35, 0.2, 1),
                },
            },
        ],
    ...
    const graph = new RenderGraph(device, "PBRTDiffusePTGraph");
    ...
    const res = await fetch("/tests/oracle/out-native/oracle-pt-pbrtdiffuse.MinimalPathTracer.color.0.exr");
    ...
    console.error(`# pbrtDiffusePT: meanAbs=${mean.toExponential(2)} rel=${rel.toExponential(2)} bad=${badPixels}`);
