/**
 * M7 material zoo: HairMaterial (Chiang16) through the static-dispatch factory
 * (WEBFALCOR_MTL_HAIR) — path-traced hair-material quad vs the native hardware-DXR
 * oracle. The scene mirrors tests/oracle/assets/oracle-pt-hair.pyscene:
 * TriangleMesh.createQuad(1,1) rotated 90° about X into [0,1]^2 at z=0.
 *
 * Regenerate the oracle with:
 *   Falcor/build/linux-gcc/bin/Debug/Mogwai --script tests/oracle/render-native-pt-hair.py --headless
 */
...
gpuTest("HairPathTracer.matchesNativeOracle", async ({ device }) => {
    const size = 256;
    ...
    const scene = new Scene(
        ...
        [
            {
                header: { materialType: MaterialType.Hair },
                basic: {
                    baseColor: new float4(0.7, 0.4, 0.2, 1),
                    specular: new float4(0.3, 0.3, 1.5, 0), // (betaM, betaN, alpha) hair params
                },
            },
        ],
    ...
    const graph = new RenderGraph(device, "HairPTGraph");
    ...
    const res = await fetch("/tests/oracle/out-native/oracle-pt-hair.MinimalPathTracer.color.0.exr");
    ...
    console.error(`# hairPT: meanAbs=${mean.toExponential(2)} rel=${rel.toExponential(2)} bad=${badPixels}`);
