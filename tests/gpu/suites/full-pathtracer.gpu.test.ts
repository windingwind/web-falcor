 ...
 * MIS, EmissiveUniformSampler) vs the native hardware-DXR PathTracer on the
 * emissive two-quad scene.
 *
 * Regenerate the oracle with:
 *   Falcor/build/linux-gcc/bin/Debug/Mogwai --script tests/oracle/render-native-fullpt.py --headless
 */
...
gpuTest("FullPathTracer.matchesNativeOracle", async ({ device }) => {
    const size = 256;
    ...
    const graph = new RenderGraph(device, "FullPTGraph");
    ...
    const res = await fetch("/tests/oracle/out-native/oracle-fullpt.PathTracer.color.0.exr");
    ...
    console.error(`# fullPT: meanAbs=${mean.toExponential(2)} rel=${rel.toExponential(2)} bad=${badPixels}`);
