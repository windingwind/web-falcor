...
| `Volumes/` GridVolumeSampler, phase functions | 🟡 in progress | NanoVDB traversal is shader code, portable. OpenVDB .vdb reference parser done (tools/vdb/vdb_parse.py: file v222, ACTIVE_MASK compression, half floats; validated byte-exact vs file metadata). ⚠ native prebuilt openvdb (vcpkg Debug AND Release) segfaults in initialize() on this machine (ODR split-brain with libopenvdb.so.9.0) -> native oracles must go through .nvdb (NanoVDB path, header-only, works). Next: NanoVDB builder, scene plumbing, PathTracer volume path |
