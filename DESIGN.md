...
| `Volumes/` GridVolumeSampler, phase functions | 🟡 in progress | Web loads the UNMODIFIED smoke.pyscene: TS OpenVDB parser + NanoVDB v32.3 builder (byte-identical to the native-validated reference tools in tools/vdb/), Grid/GridVolume hosts, pyscene bridge (GridVolume/GridSlot/loadGrid), geometry-less scenes legal. ⚠ native prebuilt openvdb segfaults in initialize() (both configs; ODR split-brain) -> native volume oracles go through .nvdb (tests/oracle/assets/smoke.nvdb, native-validated 0/500). Remaining: Scene GPU plumbing (Grid.slang binds), PathTracer USE_GRID_VOLUMES, feature oracle |
